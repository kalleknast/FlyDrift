...
**Buoyancy ($F_b$):** Modeled for the tippet nodes based on density. For the weighted fly (e.g. tungsten), buoyancy is negligible compared to gravity and is generally omitted in the simplified code, though the physics engine supports it.
...
The code uses the Turton & Levenspiel (1986) correlation for a sphere, which covers a wide range of Reynolds numbers:

$$C_{D} = \frac{24}{Re}(1 + 0.15 Re^{0.687}) + \frac{0.42}{1 + 4.25 \cdot 10^{4} Re^{-1.16}}$$

**The "Bushiness" Multiplier ($\beta$):**

To differentiate between a slim Perdigon and a fluffy Woolly Bugger:

$$C_{D,fly} = \beta \times C_{D}$$
...
Both **Normal Drag** ($F_n$) and **Tangential Drag** ($F_t$) are calculated. The Normal Drag is dominant force pushing the tippet downstream [Spolek (2007)]:

 $$F_n(y) = \frac{1}{2} \rho_{water} C_{D,norm} d_{tippet} L_{seg} |v_{norm}| v_{norm}$$

 $$F_t(y) = \frac{1}{2} \rho_{water} C_{D,tan} (\pi d_{tippet} L_{seg}) |v_{tan}| v_{tan}$$

- $d_{tippet}$: Tippet diameter.
- $C_{D,norm} \approx 1.2$, $C_{D,tan} \approx 0.01$.
...
- `simulate.py`: Contains the core physics engine (`simulate` function).
...
drift_data = simulate(
    ...
    tippet_density='nylon',     # Material density (or float in kg/m^3)
